//控制层 
app.controller('hotelController', function($scope, hotelService, uploadService) {
	...
	$scope.findHotel = function() {
		...
		hotelService.findHotel().success(
			function(response) {
	...
	$scope.findHotelSearch = function() {
	...
	$scope.findHotelPage = function() {
		...
		if($scope.skipPage!=undefined)
		{
			...
			hotelService.findHotelPage($scope.page, $scope.row, $scope.searchEntity,$scope.getCookie('token')).success(
				function(response) {
...
//查询实体 
	$scope.findHotelById = function(entity) {
		$scope.entity=entity;
	}

	// //查询实体 
	// $scope.findHotelById = function(id) {
	// 	hotelService.findHotelById(id,$scope.getCookie('token')).success(
	// 		function(response) {
	// 			if (response.code) {
	// 				$scope.code = response.code;
	// 				// if($scope.code=='200')
	...
	$scope.updateHotel = function() {
		...
		console.log(".............");
		serviceObject = hotelService.updateHotel($scope.entity,$scope.getCookie('token')); //修改  
	...
	$scope.insertHotel = function() {
		...
		serviceObject = hotelService.insertHotel($scope.entity,$scope.getCookie('token')); //修改  
	...
	//更新酒店状态
	$scope.updateHotelStatus = function() {
		...
		hotelService.updateHotelStatus($scope.entity,$scope.getCookie('token')).success(
			function(response) {
				if (response.code) {
					$scope.code = response.code;
					if ($scope.code == '200') {
	...
	$scope.deleteHotelById = function() {
		//获取选中的复选框			
		hotelService.deleteHotelById($scope.entity.order_id,$scope.getCookie('token')).success(
			function(response) {
				if (response.code) {
					$scope.code = response.code;
					if ($scope.code == '200') {
	...
	$scope.findHotelSearchReset=function(){
	...
	$scope.insertHotelClick= function() {
		$scope.entity={};
	}
});
