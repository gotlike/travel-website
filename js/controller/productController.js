//控制层 
app.controller('productController', function($scope, productService, uploadService) {
	...
	$scope.findProduct = function() {
		...
		productService.findProduct().success(
			function(response) {
	...
	$scope.findProductSearch = function() {
	...
	$scope.findProductPage = function() {
		...
		if($scope.skipPage!=undefined)
		{
			...
			productService.findProductPage($scope.page, $scope.row, $scope.searchEntity,$scope.getCookie('token')).success(
				function(response) {
...
//查询实体 
	$scope.findProductById = function(entity) {
		$scope.entity=entity;
	}

	// //查询实体 
	// $scope.findProductById = function(id) {
	// 	productService.findProductById(id,$scope.getCookie('token')).success(
	// 		function(response) {
	// 			if (response.code) {
	// 				$scope.code = response.code;
	// 				// if($scope.code=='200')
	...
	$scope.updateProduct = function() {
		...
		console.log(".............");
		serviceObject = productService.updateProduct($scope.entity,$scope.getCookie('token')); //修改  
	...
	$scope.insertProduct = function() {
		...
		serviceObject = productService.insertProduct($scope.entity,$scope.getCookie('token')); //修改  
	...
	$scope.updateProductStatus = function() {
		...
		productService.updateProductStatus($scope.entity,$scope.getCookie('token')).success(
			function(response) {
				if (response.code) {
					$scope.code = response.code;
					if ($scope.code == '200') {
	...
	$scope.deleteProductById = function() {
		//获取选中的复选框			
		productService.deleteProductById($scope.entity.order_id,$scope.getCookie('token')).success(
			function(response) {
				if (response.code) {
					$scope.code = response.code;
					if ($scope.code == '200') {
	...
	$scope.findProductSearchReset=function(){
